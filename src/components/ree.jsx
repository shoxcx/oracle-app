...
import obj_dragon from '../assets/obj_dragon.png';
import obj_herald from '../assets/obj_herald.png';
import obj_grubs from '../assets/obj_grubs.png';
import obj_nash from '../assets/obj_nash.png';
import obj_tower from '../assets/obj_tower.png';
            ...
            try {
                ...
                if (showObjectives) {
                    ...
                    if (data.events && data.events.Events) {
                        ...
                        if (barons.length > 0) baronSpawnTime = barons[barons.length - 1].EventTime + 360; // 6 min respawn (corrigé)

                        const hordes = data.events.Events.filter(e => e.EventName === 'HordeKill');
                        if (hordes.length > 0 && hordes[hordes.length - 1].EventTime < 525) grubsSpawnTime = hordes[hordes.length - 1].EventTime + 240; // 4 min respawn

                        const heralds = data.events.Events.filter(e => e.EventName === 'HeraldKill');
                        if (heralds.length > 0) heraldSpawnTime = 99999; // L'Héraut ne respawn plus
                    }
                    ...
                    if (gameTime >= drakeSpawnTime - 30 && gameTime < drakeSpawnTime + 90 && refs.notifiedSpawns.drake !== drakeSpawnTime) {
                        ...
                        pushToast({
                            type: 'objective', title: 'OBJECTIF (30s)', name: 'Dragon', status: 'Le prochain Dragon apparaît bientôt.', iconColor: 'bg-red-500', imageUrl: obj_dragon, duration: 12000
                        });
                    }
                    ...
                    if (gameTime >= baronSpawnTime - 30 && gameTime < baronSpawnTime + 90 && refs.notifiedSpawns.baron !== baronSpawnTime) {
                        ...
                        pushToast({
                            type: 'objective', title: 'OBJECTIF MAJEUR (30s)', name: 'Baron Nashor', status: 'Le Baron apparaît bientôt.', iconColor: 'bg-fuchsia-600', imageUrl: obj_nash, duration: 12000
                        });
                    }
                    ...
                    if (gameTime >= grubsSpawnTime - 30 && gameTime < grubsSpawnTime + 90 && refs.notifiedSpawns.grubs !== grubsSpawnTime && gameTime < 820) {
                        ...
                        pushToast({
                            type: 'objective', title: 'OBJECTIF (30s)', name: 'Larves du Néant', status: 'Important pour push les tours rapidement.', iconColor: 'bg-purple-700', imageUrl: obj_grubs, duration: 12000
                        });
                    }
                    ...
                    if (gameTime >= heraldSpawnTime - 30 && gameTime < heraldSpawnTime + 90 && refs.notifiedSpawns.herald !== heraldSpawnTime) {
                        ...
                        pushToast({
                            type: 'objective', title: 'OBJECTIF (30s)', name: 'Héraut de la Faille', status: 'Le Héraut apparaît dans 30s.', iconColor: 'bg-indigo-600', imageUrl: obj_herald, duration: 12000
                        });
                    }
                }
    ...
    return (
        <div style={{
            ...
            <DraggableWidget id="build_hud" defaultPosition={{ x: 24, y: 24 }} className="flex flex-col gap-4 items-start cursor-move opacity-90 hover:opacity-100 transition-opacity">
                ...
                {toasts && toasts.some(t => t.type !== 'skill' && t.type !== 'ward' && t.type !== 'objective') && (
                    <div className="flex flex-col gap-4 items-start pb-4">
                        {toasts.filter(toast => toast.type !== 'skill' && toast.type !== 'ward' && toast.type !== 'objective').map(toast => {
            ...
            {toasts && toasts.some(t => t.type === 'objective') && (
                <DraggableWidget id="objective_hud_v2" defaultPosition={{ x: 24, y: 300 }} className="flex flex-col gap-3 items-start cursor-move opacity-90 hover:opacity-100 transition-opacity">
                    {toasts.filter(toast => toast.type === 'objective').map(toast => (
                        <div
                            key={toast.id}
                            className={`w-[320px] bg-white/[0.03] backdrop-blur-[30px] border border-white/10 rounded-[24px] p-4 pt-5 pl-14 flex flex-col relative overflow-hidden shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5)] transition-all duration-500 ease-out ${toast.isLeaving ? 'opacity-0 -translate-x-8 scale-95' : 'animate-in slide-in-from-left-8 fade-in'}`}
                        >
                            {/* Accent Glow */}
                            <div className={`absolute -left-10 top-1/2 -translate-y-1/2 w-24 h-24 rounded-full blur-[40px] opacity-40 pointer-events-none ${toast.iconColor}`} />

                            {/* Image sans cadre, top-left, semi-transparente (Modifiez position et opacité ici) */}
                            {toast.imageUrl && (
                                <img src={toast.imageUrl} alt="Objective" className="absolute -top-3 -left-3 w-20 h-20 object-contain opacity-40 pointer-events-none drop-shadow-xl" />
                            )}

                            {/* Text Content */}
                            <div className="relative z-10 pl-2 pointer-events-none">
                                <div className="text-[10px] font-black text-white/50 uppercase tracking-[0.25em] mb-1 pointer-events-none">
                                    {toast.title}
                                </div>
                                <div className="text-[16px] font-black text-white/95 truncate leading-tight mb-1 drop-shadow-sm pointer-events-none">
                                    {toast.name}
                                </div>
                                <div className="text-[13px] text-white/70 font-medium leading-snug break-words pointer-events-none">
                                    {toast.status}
                                </div>
                            </div>

                            {/* Progress Bar */}
                            <div className="absolute bottom-0 left-0 h-[3px] bg-transparent w-full pointer-events-none">
                                <div
                                    className={`h-full opacity-40 rounded-full pointer-events-none ${toast.iconColor ? toast.iconColor.replace('bg-', 'text-') : 'text-white'}`}
                                    style={{ width: '100%', animation: `toastProgress ${toast.duration || 12000}ms linear forwards`, backgroundColor: 'currentColor' }}
                                />
                            </div>
                        </div>
                    ))}
