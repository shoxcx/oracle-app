...
# 1. Fixed window borders (overflow-hidden).
# 2. Fixed card text legibility (top gradient).
# 3. Removed attitude tags.
# 4. Replaced Record with Streak.
# 5. Fixed data fetching (puuid fallback).
        ...
        if (gameSession && gameSession.gameData) {
          ...
          const enrichTeam = async (lcuTeam, teamKey) => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                // FORCE PUUID LOOKUP IF NAME IS MISSING
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-5 lg:p-6 z-10">
          ...
          <div className="flex items-center justify-between border-t border-white/10 pt-4 mt-auto bg-black/60 -mx-6 -mb-6 px-6 pb-6 backdrop-blur-2xl">
            ...
            <div className="text-right">
              <span className="text-[8px] font-black text-white/40 uppercase tracking-[0.15em] mb-1">{t('streak_wins')} / {t('streak_losses')}</span>
              <span className={cn("text-xs font-black uppercase italic", p.streakType === 'win' ? "text-green-400" : "text-red-400")}>
                 {p.streak} {p.streakType === 'win' ? 'WIN' : 'LOSS'}
              </span>
            </div>
          </div>
        </div>
...
print("Loading Screen UI Final Touches Applied")
