...
# I'll replace the LoadingOverlay component with a final interactive version
new_loading_overlay = """function LoadingOverlay({ t, visualMode, theme }) {
  ...
  const closeWindow = () => {
    window.ipcRenderer.send('app:hide-loading-screen');
  };
  ...
  const PlayerCard = ({ p, team }) => {
    ...
    return (
      <div 
        className={cn(
          "relative w-full aspect-[3/4.4] rounded-[2.5rem] overflow-hidden border-2 transition-all duration-500 shadow-2xl group",
          isBlue ? "border-blue-500/30 bg-blue-500/10 hover:border-blue-400" : "border-red-500/30 bg-red-500/10 hover:border-red-400",
          "hover:scale-[1.02]"
        )}
      >
        {p.champAlias && (
        ...
        {p.streak > 1 && (
          <div className={cn(
            "absolute top-5 left-5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg z-20 animate-in slide-in-from-left duration-500",
        ...
        <div className="flex items-center justify-between mb-10 shrink-0" style={{ WebkitAppRegion: 'drag' }}>
          ...
          <div className="flex items-center gap-6" style={{ WebkitAppRegion: 'no-drag' }}>
            <div className="text-right hidden sm:block">
               <span className="text-[10px] font-black text-white/10 uppercase tracking-[0.3em] block mb-1.5">Session Protocol</span>
               <span className="text-xs font-black text-yellow-500 uppercase italic tracking-widest bg-yellow-500/10 border border-yellow-500/20 px-4 py-1.5 rounded-full shadow-[0_0_15px_rgba(234,179,8,0.1)]">Active Simulation</span>
            </div>
            <button 
              onClick={closeWindow}
...
print("Loading Screen UI & Interaction Fixed")
