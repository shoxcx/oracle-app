...
// Helper to reliably find and replace SettingView
const startMarker = "function SettingsView";
const endMarker = "function OverlayLayoutEditor";

const startIndex = content.indexOf(startMarker);
const endIndex = content.indexOf(endMarker);

if (startIndex !== -1 && endIndex !== -1) {
    // We already inserted helpers in Step 193, so we just need to replace the function body
    const newSettingsCode = `function SettingsView({ theme, setTheme, visualMode, setVisualMode, language, setLanguage, t, autoAccept, setAutoAccept, autoImportRunes, setAutoImportRunes, flashPosition, setFlashPosition, overlaySettings, setOverlaySettings, panelClass }) {
...
`;
    content = content.substring(0, startIndex) + newSettingsCode + content.substring(endIndex);
}

// FIX RANKINGS VIEW HEADER
// Search for the return statement inside RankingsView specifically
const rankingsViewStart = content.indexOf("function RankingsView");
const rankingsViewReturn = content.indexOf("return (", rankingsViewStart);
const rankingsViewHeader = content.indexOf('className={cn("p-4 rounded-[24px]', rankingsViewReturn);

if (rankingsViewHeader !== -1) {
    // Found the header, let's rewrite the line to be safe
    const lineEnd = content.indexOf('>', rankingsViewHeader) + 1;
    const newLine = 'className={cn("p-4 rounded-[24px] flex items-center justify-between shrink-0", panelClass)}>';
    content = content.substring(0, rankingsViewHeader) + newLine + content.substring(lineEnd);
}
