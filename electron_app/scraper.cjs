...
class Scraper {
    ...
    async getEsportsSchedule() {
        const cacheKey = 'esports_schedule_v14';
        const cached = this.getCachedData(cacheKey, 1800000); // 30 min max for schedule to stay fresh
        if (cached) return cached;

        const url = 'https://lolesports.com/schedule';
        console.log(`[Scraper] Fetching esports schedule from API via ${url}`);

        const result = await this.runBrowserTask(url, async (win) => {
            try {
                // Fetch direct json
                const data = await win.webContents.executeJavaScript(`
                    fetch('https://esports-api.lolesports.com/persisted/gw/getSchedule?hl=en-GB', {
                ...
                if (!data || data.error || !data.data || !data.data.schedule) return [];
                ...
                const upcoming = events.filter(e => {
                    ...
                    if (e.state === 'completed') return false;
                    const eventDate = new Date(e.startTime);
                    // allow matches that started recently or will start
                    return eventDate >= new Date(now.getTime() - 6 * 3600 * 1000); // within last 6 hrs or future
                });

                const formatted = upcoming.slice(0, 50).map(e => {
                    ...
                    return {
                        team1: t1 ? (t1.code || t1.name || "TBD") : "TBD",
                        team2: t2 ? (t2.code || t2.name || "TBD") : "TBD",
                        logo1: t1?.image ? t1.image.replace('http://', 'https://') : null,
                        logo2: t2?.image ? t2.image.replace('http://', 'https://') : null,
                        time: timeStr,
                        date: dateStr,
                        league: e.league?.name || "INTL",
                        url: "https://lolesports.com/schedule"
                    };
                });
